...
IV_Data = np.load("2N7000A_IV_Data_ID_VD.npz")
#IV_Data = np.load("2N7000A_IV_Data_Kethley.npz")
...
Imax = 0; Imin = 0;
for j in range(N_G):
    plt.scatter(V_D_V_G_array[j], I_D_V_G_array[j], label = f"V_G = {V_G_initial + j*V_G_inc}")
    #plt.scatter(V_app_array, V_CH2_array, label = "V_CH2")
    Imax = max(Imax, max(I_D_V_G_array[j])); Imin = min(Imin, min(I_D_V_G_array[j]));

Ipp = abs(Imax - Imin)
plt.ylim(Imin - 0.1*Ipp, Imax + 0.1*Ipp)
plt.xlabel("Drain Voltage, V_D(V)")
plt.ylabel("Drain Current, I_D(A)")
plt.title("2N7000A I_D V_D Characteristics")
