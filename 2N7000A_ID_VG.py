...
for j in range(N_D):
    ...
    if plot_every_sweep:
        ...
        plt.title(f"2N7000A NMOS I_D vs V_G for V_D = {V_D}")
...
#Save the data
np.savez("2N7000A_IV_Data_ID_VG.npz", V_G_V_D_array=V_G_V_D_array, I_D_V_D_array=I_D_V_D_array, V_D_initial=V_D_initial, V_D_inc=V_D_inc)
...
plt.xlabel("Gate Voltage, V_G(V)")
plt.ylabel("Drain Current, I_D(A)")
plt.title(f"2N7000A NMOS I_D vs V_G")
plt.grid(True); plt.legend(); 
plt.show()
###########################################################################################################
ser.close()
